...
# 2. 计算与处理函数
def calculate_stats(scores: List[int]) -> Dict[str, Union[float, int]]:
    """计算总分、平均分、最高分以及不及格人数"""
...
# 3. 输出与展示函数
def print_report(scores: List[int], stats: Dict[str, Union[float, int]]) -> None:
    """打印最终的统计结果"""
    print(f"成绩列表: {scores}")
...
if __name__ == "__main__":
    ...
    # 步骤二：计算结果
    result_stats: Dict[str, Union[float, int]] = calculate_stats(my_scores)
